...
_ES_PLUGIN_ID = "force.bdss.es.plugin.example.v0.factory"


class ESContributedUI(ContributedUI):
    #: Name for the UI in selection screen
    name = "ES Example Workflow"

    #: Description of the UI
    desc = (
        "A toy model for mF which calculate surface and volume of a cone"
    )
    ...
    def _mco_data(self):
        return {
            "id": '.'.join([_ES_PLUGIN_ID, "es_mco"]),
            "model_data": {
                "parameters": [
                    {
                        "id": ".".join([
                            _ES_PLUGIN_ID, "es_mco",
                            "parameter", "ranged"
                        ]),
                        "model_data": {
